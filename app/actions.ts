...
export async function createInvoice(prevState:any, formData:FormData){
  ...
  const items = submission.value.items.map((item: { description: string; rate: number; quantity: number }) => ({
...
export async function updateInvoice(prevState: any, formData: FormData) {
    ...
    emailClient.send({
        ...
        template_variables: {
            "clientName": submission.value.clientName,
            "invoiceNumber": submission.value.invoiceNumber,
            "dueDate": new Intl.DateTimeFormat('en-US', { dateStyle: 'long' }).format(new Date(submission.value.dueDate)),
            "totalAmount": formatCurrency({ amount: submission.value.total, currency: submission.value.currency as any }),
            "invoiceLink": `http://localhost:3000/api/invoice/${data.id}`
        },
    });
